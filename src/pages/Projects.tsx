	...
	return (
		<section className="max-w-2xl mx-auto mt-10 px-4 space-y-6">
			...
			<p className="text-sm text-gray-700 leading-relaxed">
				A few selected projects that reflect my focus on embedded ML, robotics, and real-time perception
				systems.
				Some are research-inspired; all are grounded in deployment constraints.
			</p>

			<ul className="space-y-6 text-sm text-gray-700">
				{ projects.map((project) => (
					<li key={ project.name }>
						<h2 className="font-medium text-gray-900 flex items-baseline gap-2">
							{ project.name }
							{ project.link && (
								<a
									href={ project.link }
									...
									className="text-sm text-gray-500 hover:text-gray-800 tracking-tight font-normal"
								>
									github
								</a>
							) }
